...
const filteredUserData = computed(() => {
    const fEmail = userData.value.filter(v => !email.value || v.email.toLowerCase().includes(email.value.toLowerCase()));
    const fPhone = fEmail.filter(v => !phone.value || v.phone.toLowerCase().includes(phone.value.toLowerCase()));
    const fFirstName = fPhone.filter(v => !firstName.value || v.firstName.toLowerCase().includes(firstName.value.toLowerCase()));
    const fLastName = fFirstName.filter(v => !lastName.value || v.lastName.toLowerCase().includes(lastName.value.toLowerCase()));
    const fAge = fLastName.filter(v => !age.value || v.age === age.value);
    const fCountry = fAge.filter(v => !country.value || v.country.toLowerCase().includes(country.value.toLowerCase()));
...
const isEditValid = computed(() => {
    try {
        ...
        if(arr.some(v => typeof v !== 'string')) {
            editErr.value = 'Elements of courses should be strings. Did you forget "quotes"?';
            return false;
        }
...
function editCourses(email: string) {
    ...
    currEditCourses.value = JSON.stringify(userData.value.find(v => v.email === email)!.courses);
}
...
<template>
<div class="flex gap-4 min-h-screen p-4 font-mono">
    <div class="relative flex flex-col gap-4 w-1/3 grow p-4 rounded-md border border-dashed border-primary">
    ...
    <div class="w-2/3 grow p-4 rounded-md border border-dashed border-primary">
                    ...
                    <UCard>
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <UInput 
                                ...
                                class="rounded-md" :class="{ 'ring-2 ring-rose-500': !isEditValid }"
                                outline="white"
                            />
                            <div class="flex gap-4 justify-end">
                                <UButton @click="editing[user.email] = false">Cancel</UButton>
                                <UButton :disabled="!isEditValid" :loading="editSaving" @click="saveEdit">Save</UButton>
                            </div>
                        </div>
                    </UCard>
                </UModal>
